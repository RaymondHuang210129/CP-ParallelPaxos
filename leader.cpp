...
Leader::Leader(Entry myEntry, int numberOfCommander, int threadID,
               std::vector<Entry>& replicas, std::vector<Entry>& acceptors) {
    ...
    this->commanderStartPort = myEntry.threadStartPort + myEntry.numThreads +
                               threadID * numberOfCommander;
    for (int i = 0; i < this->numberOfCommander; ++i) {
        int commanderPort = commanderStartPort + i;
        commanderThreads.emplace_back([commanderPort, threadID, &replicas,
                                       &acceptors]() {
...
void Leader::run(void* arg) {
    ...
    while (!shouldTerminate) {
        ...
        if (propose != nullptr) {
            // std::cout << "Leader Receive Propose message " <<
            // propose->serialize() << std::endl;
            Assign assign(propose->getSlot(), propose->getCommand());
            node->send_data(
                "127.0.0.1",
                commanderStartPort + (assignCommander % numberOfCommander),
                assign.serialize());
        }
        delete m;
        assignCommander += 1;
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cout << "Invalid arguments count. Should enter server [leader ip] "
                     "[leader port] [number of commander]\n "
                  << std::endl;
        exit(1);
    }
    ...
    for (int i = 0; i < myEntry.numThreads; i++) {
        leaderThreads.emplace_back([&leaders, &numberOfCommander, i, &myEntry,
                                    &replicas, &acceptors]() {
